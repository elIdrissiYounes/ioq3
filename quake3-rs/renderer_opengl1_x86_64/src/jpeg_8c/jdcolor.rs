...
unsafe extern "C" fn gray_rgb_convert(
    ...
    loop {
        ...
        input_row = input_row.wrapping_add(1);
        inptr = *(*input_buf.offset(0)).offset(fresh2 as isize);
        ...
        while col < num_cols {
            ...
            let ref mut fresh4 = *outptr.offset(2);
            *fresh4 = *inptr.offset(col as isize);
            let ref mut fresh5 = *outptr.offset(1);
            *fresh5 = *fresh4;
            *outptr.offset(0) = *fresh5;
            outptr = outptr.offset(3);
            col = col.wrapping_add(1)
        }
    }
        ...
        1 => {
                ...
                || (*cinfo).jpeg_color_space == crate::jpeglib_h::JCS_YCbCr
            {
                ...
                while ci < (*cinfo).num_components {
                    (*(*cinfo).comp_info.offset(ci as isize)).component_needed = 0;
                    ci += 1
                }
            } else {
                (*(*cinfo).err).msg_code =
