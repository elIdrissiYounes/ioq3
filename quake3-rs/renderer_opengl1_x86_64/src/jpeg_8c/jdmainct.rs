 ...
 */
{
    ...
    while ci < (*cinfo).num_components {
        ...
        let ref mut fresh0 = *(*main_ptr).xbuffer[0].offset(ci as isize);
        *fresh0 = xbuf;
        xbuf = xbuf.offset((rgroup * (M + 4)) as isize);
        let ref mut fresh1 = *(*main_ptr).xbuffer[1].offset(ci as isize);
        *fresh1 = xbuf;
        ci += 1;
 ...
 */
{
    ...
    while ci < (*cinfo).num_components {
        ...
        i = 0;
        while i < rgroup * 2 {
            let ref mut fresh4 = *xbuf1.offset((rgroup * (M - 2) + i) as isize);
            *fresh4 = *buf.offset((rgroup * M + i) as isize);
            let ref mut fresh5 = *xbuf1.offset((rgroup * M + i) as isize);
            *fresh5 = *buf.offset((rgroup * (M - 2) + i) as isize);
            i += 1
        }
...
unsafe extern "C" fn process_data_simple_main(
    ...
    if (*main_ptr).buffer_full == 0 {
        ...
        (*main_ptr).buffer_full = 1 /* suspension forced, can do nothing more */
...
unsafe extern "C" fn process_data_context_main(
    ...
    if (*main_ptr).buffer_full == 0 {
        ...
        /* count rows received */
        (*main_ptr).buffer_full = 1; /* OK, we have an iMCU row to work with */
...
pub unsafe extern "C" fn jinit_d_main_controller(
    ...
    if (*(*cinfo).upsample).need_context_rows != 0 {
        if (*cinfo).min_DCT_v_scaled_size < 2 {
            ...
            (*(*cinfo).err).msg_code = crate::src::jpeg_8c::jerror::JERR_NOTIMPL as i32; /* Alloc space for xbuffer[] lists */
            Some(
                (*(*cinfo).err)
                    .error_exit
                    .expect("non-null function pointer"),
