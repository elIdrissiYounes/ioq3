...
pub static mut crctable: [u16; 257] = [
    0, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129, 0xa14a, 0xb16b,
...
pub unsafe extern "C" fn CRC_ProcessByte(
    ...
    *crcvalue = ((*crcvalue as i32) << 8
        ^ crctable[(*crcvalue as i32 >> 8 ^ data as i32) as usize] as i32) as u16;
}
...
pub unsafe extern "C" fn CRC_Value(mut crcvalue: u16) -> u16 {
    return (crcvalue as i32 ^ 0) as u16;
}
...
pub unsafe extern "C" fn CRC_ContinueProcessString(
    ...
    while i < length {
        *crc = ((*crc as i32) << 8
            ^ crctable[(*crc as i32 >> 8 ^ *data.offset(i as isize) as i32) as usize] as i32)
            as u16;
        i += 1
    }
    //end for
