...
static mut afd: aviFileData_t = aviFileData_t {
    ...
    chunkStack: [0; 16],
    chunkStackTop: 0,
    cBuffer: 0 as *mut crate::src::qcommon::q_shared::byte,
    eBuffer: 0 as *mut crate::src::qcommon::q_shared::byte,
};
...
unsafe extern "C" fn SafeFS_Write(
    ...
    if crate::src::qcommon::files::FS_Write(buffer_0, len, f) < len {
        crate::src::qcommon::common::Com_Error(
            ...
            b"Failed to write avi file\x00" as *const u8 as *const i8,
        );
    };
}
/*
...
unsafe extern "C" fn WRITE_STRING(mut s: *const i8) {
    crate::stdlib::memcpy(
...
unsafe extern "C" fn WRITE_4BYTES(mut x: i32) {
    buffer[(bufIndex + 0) as usize] = (x >> 0 & 0xffi32) as crate::src::qcommon::q_shared::byte;
    buffer[(bufIndex + 1) as usize] = (x >> 8 & 0xffi32) as crate::src::qcommon::q_shared::byte;
    buffer[(bufIndex + 2) as usize] = (x >> 16 & 0xffi32) as crate::src::qcommon::q_shared::byte;
    buffer[(bufIndex + 3) as usize] = (x >> 24 & 0xffi32) as crate::src::qcommon::q_shared::byte;
    bufIndex += 4;
}
/*
===============
...
unsafe extern "C" fn START_CHUNK(mut s: *const i8) {
    if afd.chunkStackTop == 16 {
        crate::src::qcommon::common::Com_Error(
            crate::src::qcommon::q_shared::ERR_DROP as i32,
            b"ERROR: Top of chunkstack breached\x00" as *const u8 as *const i8,
        );
    }
...
unsafe extern "C" fn END_CHUNK() {
    ...
    if afd.chunkStackTop <= 0 {
        crate::src::qcommon::common::Com_Error(
            crate::src::qcommon::q_shared::ERR_DROP as i32,
            b"ERROR: Bottom of chunkstack breached\x00" as *const u8 as *const i8,
        );
    }
    afd.chunkStackTop -= 1;
...
pub unsafe extern "C" fn CL_WriteAVIHeader() {
    bufIndex = 0; //"avih" "chunk" size
    afd.chunkStackTop = 0; //dwMicroSecPerFrame
    START_CHUNK(b"RIFF\x00" as *const u8 as *const i8); //dwMaxBytesPerSec
    WRITE_STRING(b"AVI \x00" as *const u8 as *const i8); //dwReserved1
    START_CHUNK(b"LIST\x00" as *const u8 as *const i8); //dwFlags bits HAS_INDEX and IS_INTERLEAVED
    WRITE_STRING(b"hdrl\x00" as *const u8 as *const i8); //dwTotalFrames
    WRITE_STRING(b"avih\x00" as *const u8 as *const i8); //dwInitialFrame
    WRITE_4BYTES(56);
    ...
    if afd.audio as u64 != 0 {
        //dwStreams
        WRITE_4BYTES(2i32); //dwSuggestedBufferSize
    } else {
        WRITE_4BYTES(1i32); //dwWidth
    } //dwHeight
    ...
    WRITE_4BYTES(0);
    WRITE_4BYTES(0);
    START_CHUNK(b"LIST\x00" as *const u8 as *const i8);
    WRITE_STRING(b"strl\x00" as *const u8 as *const i8);
    WRITE_STRING(b"strh\x00" as *const u8 as *const i8);
    //biClrImportant
    WRITE_4BYTES(56); //"strh" "chunk" size
    WRITE_STRING(b"vids\x00" as *const u8 as *const i8); // BI_RGB
    if afd.motionJpeg as u64 != 0 {
        WRITE_STRING(b"MJPG\x00" as *const u8 as *const i8); //dwFlags
    } else {
        WRITE_4BYTES(0i32); //dwPriority
    } //dwInitialFrame
    WRITE_4BYTES(0); //dwTimescale
    WRITE_4BYTES(0); //dwDataRate
    WRITE_4BYTES(0); //dwStartTime
    WRITE_4BYTES(1); //dwDataLength
    ...
    WRITE_4BYTES(-(1)); //rcFrame
    WRITE_4BYTES(0); //rcFrame
    WRITE_2BYTES(0); //rcFrame
    WRITE_2BYTES(0); //"strf" "chunk" size
    WRITE_2BYTES(afd.width); //biSize
    WRITE_2BYTES(afd.height); //biWidth
    WRITE_STRING(b"strf\x00" as *const u8 as *const i8); //biHeight
    WRITE_4BYTES(40); //biPlanes
    WRITE_4BYTES(40); //biBitCount
    ...
    if afd.motionJpeg as u64 != 0 {
        //biCompression
        WRITE_STRING(b"MJPG\x00" as *const u8 as *const i8);
        WRITE_4BYTES(afd.width * afd.height);
    //biSizeImage
    } else {
        WRITE_4BYTES(0);
        WRITE_4BYTES(afd.width * afd.height * 3i32); // BI_RGB
                                                     //biSizeImage
    } //biXPelsPetMeter
    ...
    if afd.audio as u64 != 0 {
        ...
        //cbSize
        WRITE_4BYTES(56); //"strh" "chunk" size
        WRITE_STRING(b"auds\x00" as *const u8 as *const i8); //FCC
        ...
        WRITE_4BYTES(0); //dwDataLength
        WRITE_4BYTES(afd.a.totalBytes / afd.a.sampleSize); //dwSuggestedBufferSize
        WRITE_4BYTES(0); //dwQuality
        WRITE_4BYTES(-(1)); //dwSampleSize
        WRITE_4BYTES(afd.a.sampleSize); //rcFrame
        WRITE_2BYTES(0); //rcFrame
        WRITE_2BYTES(0); //rcFrame
        WRITE_2BYTES(0); //rcFrame
        WRITE_2BYTES(0); //"strf" "chunk" size
        WRITE_STRING(b"strf\x00" as *const u8 as *const i8); //wFormatTag
        WRITE_4BYTES(18); //nChannels
    ...
    START_CHUNK(b"LIST\x00" as *const u8 as *const i8);
    WRITE_STRING(b"movi\x00" as *const u8 as *const i8);
}
/*
===============
...
pub unsafe extern "C" fn CL_OpenAVIForWriting(
    ...
    crate::stdlib::memset(
        ...
        0,
        ::std::mem::size_of::<aviFileData_t>(),
    );
    // Don't start if a framerate has not been chosen
    if (*crate::src::client::cl_main::cl_aviFrameRate).integer <= 0 {
    ...
    afd.idxF = crate::src::qcommon::files::FS_FOpenFileWrite(crate::src::qcommon::q_shared::va(
        b"%s.index.dat\x00" as *const u8 as *mut i8,
        fileName,
    ));
    if afd.idxF <= 0 {
    ...
    crate::src::qcommon::q_shared::Q_strncpyz(afd.fileName.as_mut_ptr(), fileName, 64);
    afd.frameRate = (*crate::src::client::cl_main::cl_aviFrameRate).integer;
    afd.framePeriod = (1000000.0 / afd.frameRate as f32) as i32;
    ...
    afd.cBuffer =
        crate::src::qcommon::common::Z_Malloc((afd.width * 3 + 16 - 1) * afd.height + 16 - 1)
            as *mut crate::src::qcommon::q_shared::byte;
    // raw avi files have pixel lines start on 4-byte boundaries
    afd.eBuffer =
        crate::src::qcommon::common::Z_Malloc((afd.width * 3 + 4 - 1 & !(4 - 1)) * afd.height)
            as *mut crate::src::qcommon::q_shared::byte;
    ...
    if afd.a.rate % afd.frameRate != 0 {
        ...
        while afd.a.rate % suggestRate != 0 && suggestRate >= 1 {
            suggestRate -= 1
        }
        crate::src::qcommon::common::Com_Printf(
    ...
    ) == 0
    {
        ...
        crate::src::qcommon::cvar::Cvar_VariableString(b"s_backend\x00" as *const u8 as *const i8),
    ...
    bufIndex = 0;
    START_CHUNK(b"idx1\x00" as *const u8 as *const i8);
    SafeFS_Write(
...
pub unsafe extern "C" fn CL_WriteAVIVideoFrame(
    ...
    let mut chunkOffset: i32 = afd.fileSize - afd.moviOffset - 8;
    let mut chunkSize: i32 = 8 + size;
    let mut paddingSize: i32 = (size + 2 - 1 & !(2 - 1)) - size;
    let mut padding: [crate::src::qcommon::q_shared::byte; 4] = [0, 0, 0, 0];
    if afd.fileOpen as u64 == 0 {
        return;
    }
    // Chunk header + contents + padding
    if CL_CheckFileSize(8 + size + 2) as u64 != 0 {
        return;
    }
    bufIndex = 0;
    WRITE_STRING(b"00dc\x00" as *const u8 as *const i8);
    WRITE_4BYTES(size);
    SafeFS_Write(buffer.as_mut_ptr() as *const libc::c_void, 8, afd.f);
    ...
    // Index
    bufIndex = 0; //dwIdentifier
    WRITE_STRING(b"00dc\x00" as *const u8 as *const i8); //dwFlags (all frames are KeyFrames)
    WRITE_4BYTES(0x10); //dwOffset
    WRITE_4BYTES(chunkOffset); //dwLength
    WRITE_4BYTES(size);
    SafeFS_Write(buffer.as_mut_ptr() as *const libc::c_void, 16, afd.idxF);
    afd.numIndices += 1;
}
/*
===============
...
pub unsafe extern "C" fn CL_WriteAVIAudioFrame(
    ...
    static mut bytesInBuffer: i32 = 0;
    if afd.audio as u64 == 0 {
        return;
    }
    if afd.fileOpen as u64 == 0 {
    ...
    // Chunk header + contents + padding
    if CL_CheckFileSize(8 + bytesInBuffer + size + 2) as u64 != 0 {
        return;
    }
    if bytesInBuffer + size > 44100 {
        crate::src::qcommon::common::Com_Printf(
            b"^3WARNING: Audio capture buffer overflow -- truncating\n\x00" as *const u8
                as *const i8,
        );
        size = 44100 - bytesInBuffer
    }
    crate::stdlib::memcpy(
    ...
    {
        let mut chunkOffset: i32 = afd.fileSize - afd.moviOffset - 8;
        let mut chunkSize: i32 = 8 + bytesInBuffer;
        let mut paddingSize: i32 = (bytesInBuffer + 2 - 1 & !(2 - 1)) - bytesInBuffer;
        let mut padding: [crate::src::qcommon::q_shared::byte; 4] = [0, 0, 0, 0];
        bufIndex = 0;
        WRITE_STRING(b"01wb\x00" as *const u8 as *const i8);
        ...
        // Index
        bufIndex = 0; //dwIdentifier
        WRITE_STRING(b"01wb\x00" as *const u8 as *const i8); //dwFlags
        WRITE_4BYTES(0); //dwOffset
        WRITE_4BYTES(chunkOffset); //dwLength
        WRITE_4BYTES(bytesInBuffer);
        SafeFS_Write(buffer.as_mut_ptr() as *const libc::c_void, 16, afd.idxF);
        afd.numIndices += 1;
...
pub unsafe extern "C" fn CL_CloseAVI() -> crate::src::qcommon::q_shared::qboolean {
    ...
    let mut indexSize: i32 = afd.numIndices * 16;
    let mut idxFileName: *const i8 = crate::src::qcommon::q_shared::va(
        b"%s.index.dat\x00" as *const u8 as *mut i8,
        afd.fileName.as_mut_ptr(),
    );
    // AVI file isn't open
    ...
    while indexRemainder > 2048 {
        crate::src::qcommon::files::FS_Read(
            buffer.as_mut_ptr() as *mut libc::c_void,
            2048,
            afd.idxF,
        );
        SafeFS_Write(buffer.as_mut_ptr() as *const libc::c_void, 2048, afd.f);
        afd.fileSize += 2048;
        indexRemainder -= 2048
    }
    crate::src::qcommon::files::FS_Read(
    ...
    bufIndex = 4;
    WRITE_4BYTES(afd.fileSize - 8);
    bufIndex = afd.moviOffset + 4;
